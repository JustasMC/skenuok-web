...
export function GeneratorWorkspacePanel({ history, credits, sessionMode }: Props) {
  const t = useDict().tools.generatorUi.workspace;
  ...
  const subtitle = sessionMode === "user" ? t.subtitleUser : t.subtitleSession;
  const dateLocale = locale === "en" ? "en-GB" : "lt-LT";

  function formatMinutes(totalMin: number): string {
    if (totalMin < 60) return t.timeMin.replace("{n}", String(Math.round(totalMin)));
    const h = Math.floor(totalMin / 60);
    const m = Math.round(totalMin % 60);
    return m > 0
      ? t.timeHoursMin.replace("{h}", String(h)).replace("{m}", String(m))
      : t.timeHours.replace("{h}", String(h));
  }
    ...
    <div className="space-y-6">
      <div className="grid min-w-0 gap-3 sm:grid-cols-3">
        <div className="min-w-0 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-3 sm:p-4">
          <p className="text-[11px] font-medium uppercase tracking-wide text-zinc-500">{t.creditsLeft}</p>
          <p className="mt-1 font-mono text-xl font-semibold tabular-nums text-[var(--color-lime)] sm:text-2xl">{credits ?? "—"}</p>
        </div>
        <div className="min-w-0 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-3 sm:p-4">
          <p className="text-[11px] font-medium uppercase tracking-wide text-zinc-500">{t.avgSeo}</p>
          <p className="mt-1 font-mono text-xl font-semibold tabular-nums text-[var(--color-electric)] sm:text-2xl">
            {avg != null ? `${avg}` : "—"}
          </p>
          <p className="mt-1 text-[11px] text-zinc-600">
            {n > 0 ? t.fromArticles.replace("{n}", String(n)) : t.noDataYet}
          </p>
        </div>
        <div className="min-w-0 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-3 sm:p-4">
          <p className="text-[11px] font-medium uppercase tracking-wide text-zinc-500">{t.timeSaved}</p>
          ...
          <p className="mt-1 text-[11px] text-zinc-600">
            {t.perArticle.replace("{n}", String(MINUTES_SAVED_PER_ARTICLE))}
          </p>
        </div>
      </div>
      ...
      <div>
        <div className="mb-3 flex flex-wrap items-end justify-between gap-2">
          <div>
            <h3 className="text-sm font-semibold text-white">{t.recentTitle}</h3>
            ...
            {history.map((h) => (
              <li key={h.id} className="flex flex-col gap-1 px-3 py-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between sm:gap-2 sm:px-4">
                ...
                <span className="shrink-0 font-mono text-xs text-zinc-500">
                  {t.seoScore} {h.seoScore ?? "—"} · {new Date(h.createdAt).toLocaleString(dateLocale)}
                </span>
              </li>
            ))}
          </ul>
